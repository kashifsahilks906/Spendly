...
Spendly's currency is **PKR**. Render every monetary amount as `PKR 1,234.00`
(thousands separator, two decimals). Never use the `₹` or `$` symbol in markup,
mockups, or examples. In templates, format amounts through the shared `pkr`
Jinja filter (`{{ amount | pkr }}`).
...
- Amount field gets a `PKR` prefix and tabular-nums
